...
public class PlayerListener implements Listener {
	...
	public void onPlayerRespawnEvent(PlayerRespawnEvent event) {
		...
		Location respawnLocation = this.regionContainer.createQuery().queryValue(localPlayer.getLocation(), localPlayer,
				Flags.RESPAWN_LOCATION);
	...
	public void onPlayerItemConsumeEvent(PlayerItemConsumeEvent event) {
		...
		if (itemMeta instanceof PotionMeta potionMeta) {
			...
			this.sessionManager.get(this.worldGuardPlugin.wrapPlayer(player)).getHandler(GiveEffectsFlagHandler.class)
					.drinkPotion(player, effects);
		} else {
			Material material = event.getItem().getType();
			if (material == Material.MILK_BUCKET) {
				this.sessionManager.get(this.worldGuardPlugin.wrapPlayer(player))
						.getHandler(GiveEffectsFlagHandler.class).drinkMilk(player);
			}
		}
	}
	...
	private void checkFlyStatus(Player player, Boolean originalValueOverwrite) {
		FlyFlagHandler flyFlagHandler = this.sessionManager.get(this.worldGuardPlugin.wrapPlayer(player))
				.getHandler(FlyFlagHandler.class);
